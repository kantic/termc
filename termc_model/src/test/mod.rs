...
use math_result::MathResultType;
...
fn tst_get_result() {
    ...
    let result = get_result("0.5 - 1.8i", & context);
    assert!(result.is_ok());
    let result = result.ok().unwrap();
    ...
    let result = get_result(".458+.97i", & context);
    assert!(result.is_ok());
    let result = result.ok().unwrap();
    assert!(result.result_type == MathResultType::Complex);
    ...
    let result = get_result("(-1*-1+1*e^(5/7))/(cos(pi/(7+2i))*8+2)", & context);
    assert!(result.is_ok());
    let result = result.ok().unwrap();
    assert!(result.result_type == MathResultType::Complex);
    ...
    // test result of a expression to be complex if any operand is complex
    let result = get_result("sinh(3) - cos(pi/e) + .5i", & context);
    assert!(result.is_ok());
    let result = result.ok().unwrap();
    assert!(result.result_type == MathResultType::Complex);
